...
export class StakingPool extends Contract {
  ...
  feeSinkAddr = TemplateVar<Address>('feeSinkAddr')
  ...
  initStorage(mbrPayment: gtxn.PaymentTxn): void {
    ...
    const PoolInitMbr = ALGORAND_ACCOUNT_MIN_BALANCE + extraMBR
  ...
  linkToNFD(nfdAppId: uint64, nfdName: string): void {
    ...
    itxn.applicationCall({
      ...
      appArgs: ['verify_nfd_addr', nfdName, op.itob(nfdAppId), new Address(Global.currentApplicationAddress).bytes],
      apps: [nfdAppId],
    })
  }
  ...
  private setRoundsPerDay() {
    ...
    // const avgBlockTimeTenths = blocks[Global.round - 2].timestamp - blocks[Global.round - 11].timestamp
