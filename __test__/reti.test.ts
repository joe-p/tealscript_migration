...
import { ValidatorRegistryClient } from '../smart_contracts/artifacts/reti/ValidatorRegistryClient.ts'
...
describe('ValidatorRegistryClient', () => {
  it('bytecode is smaller or equal to TEALScript', async () => {
    ...
    const puyaClient = new ValidatorRegistryClient({
      appId: 0n,
      defaultSender,
      algorand,
    }).appClient
    ...
    const puyaResult = await puyaClient.compile({
      deployTimeParams: { NFD_REGISTRY_APP_ID: 0 },
    })

    const tealScriptClient = new AppClient({
      appId: 0n,
      // @ts-expect-error: JSON error
      appSpec: validatorRegistryTealScript,
      algorand,
      defaultSender,
    })

    const tealScriptResult = await tealScriptClient.compile({
      deployTimeParams: { nfdRegistryAppId: 0 },
    })
